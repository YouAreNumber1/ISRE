	...
	public partial class ISRE0000 : Page
	{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			{//////// first load
				...
				List_Activities= GetProcess_ActivityList();
			}
			else
			{
		...
		{
			dynamic List_CityList1 = GetQueryDB("Home_ISRE_ACTIVITY_MAIN", "CityList");
			...
			dynamic OBJ_NOList1 = GetQueryDB("Home_ISRE_ACTIVITY_MAIN", "OBJ_NO");
			// ddlNOList
			...
			dynamic List_ACT_TYPE1 = GetQueryDB("Home_ISRE_ACTIVITY_MAIN", "ACT_TYPE");
			// ddlActTypeList
			lis_DataList = new List<CboDataList>
			{
				DefaultVal
		...
		private string ProcessLiteralSearchCriteria()
		{
			...
			foreach (Control myControl in PageNow.Controls)
			{
				...
				{
					sValue = ((ListControl)myControl).SelectedValue;

					if (sValue != "" && sValue != null)
					{
						ComSearchCriteriaData += CombineSearchCriteriaData(((ListControl)myControl).SelectedItem.Text);
					}
				...
				{
					sValue = ((TextBox)myControl).Text;
					if (sValue != "" && sValue != null)
					{
						ComSearchCriteriaData += CombineSearchCriteriaData(sValue);
					}
		...
		public static string CombineSearchCriteriaData(string SearchCriteriaDatas = "")
		{
			...
			return String.Format("<span class=\"badge bg-secondary  \">{0}</span>&nbsp;", SearchCriteriaDatas);
		}
